...
### Some example calls: Happy Path

We recommend using the provided POSTMAN collection to make things easy, but you can also use `curl` 
or any other tool that you'd like. `POST` the following JSON to `/bankapi/account`
...
to endpoints that will get you closely related resources (we use [SpringHATEOAS](https://spring.io/projects/spring-hateoas) to render
these links, details are in the `AccountModelAssembler` and `TransactionModelAssembler` classes):
...
of Mockito, while integration tests load the spring context and use the default embedded
H2 database to persist DTOs, so they naturally take more time to run.

## Logging

We use some basic AOP features to enable logging of all `public` methods at the controller,
