...
public class TransactionServiceUnitTests {
  ...
  public void whenSanityCheckerThrowsInsufficientBalanceException_thenExceptionBubblesUp() {
    ...
    doThrow(
            new InsufficientBalanceException(
                TEST_ACCOUNT_ONE.getId(),
                BigDecimal.ZERO,
                TEST_ACCOUNT_ONE.getCurrency(),
  ...
  public void whenGettingAStoredTransactionById_thenTransactionIsReturned() {
    when(transactionRepository.findById(TEST_TRANSACTION_DTO_ONE.getId()))
        .thenReturn(Optional.of(TEST_TRANSACTION_ONE));
    assertEquals(
        TEST_TRANSACTION_DTO_ONE,
  ...
  public void whenGettingAllTransactions_AllTransactionsAreReturned() {
    when(transactionRepository.findAll())
        .thenReturn(
            List.of(
                TEST_TRANSACTION_ONE,
                TEST_TRANSACTION_TWO,
                TEST_TRANSACTION_THREE,
                TEST_TRANSACTION_FOUR));
    assertTrue(
  ...
  public void whenGettingAllTransactionsFromASourceAccount_thenOnlyThoseAreReturned() {
    ...
    when(transactionRepository.findBySourceAccountId(TEST_ACCOUNT_DTO_ONE.getId()))
        .thenReturn(List.of(TEST_TRANSACTION_ONE, TEST_TRANSACTION_TWO, TEST_TRANSACTION_THREE));
    assertTrue(
        CollectionUtils.isEqualCollection(
            transactionService.getAllTransactionsFrom(TEST_ACCOUNT_DTO_ONE.getId()),
            List.of(
  ...
  public void whenGettingAllTransactionsToATargetAccount_thenOnlyThoseAreReturned() {
    ...
    when(transactionRepository.findByTargetAccountId(TEST_ACCOUNT_DTO_TWO.getId()))
        .thenReturn(List.of(TEST_TRANSACTION_ONE, TEST_TRANSACTION_TWO, TEST_TRANSACTION_THREE));
    assertTrue(
        CollectionUtils.isEqualCollection(
            transactionService.getAllTransactionsTo(TEST_ACCOUNT_DTO_TWO.getId()),
            List.of(
                TEST_TRANSACTION_DTO_ONE, TEST_TRANSACTION_DTO_TWO, TEST_TRANSACTION_DTO_THREE)));
  }
  ...
  public void
    ...
    when(transactionRepository.findBySourceAccountIdAndTargetAccountId(
            TEST_ACCOUNT_DTO_ONE.getId(), TEST_ACCOUNT_DTO_TWO.getId()))
        .thenReturn(List.of(TEST_TRANSACTION_ONE, TEST_TRANSACTION_TWO));
    assertTrue(
        CollectionUtils.isEqualCollection(
